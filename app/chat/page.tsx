...
export default function ChatPage() {
  ...
  const [cfg,setCfg] = useState<ApiCfg>(()=>(
    JSON.parse(
      localStorage.getItem('apiCfg') ||
      '{"provider":"gemini","model":"gemini-1.5-flash","keys":{"gemini":"","openai":"","groq":"","anthropic":""}}'
    )
  ));
  const saveCfg = (p: Partial<ApiCfg>)=>{
    const next={...cfg,...p};
  ...
  /* ---------- load ---------- */
  useEffect(()=>{(async()=>{
    let proj=(await supabase.from('project').select('*').order('id')).data??[];
    if(!proj.length){
      const { data } = await supabase.from('project')
        .insert({name:'Default'}).select().single();
      proj = data ? [data] : [];
    }
    setProjects(proj);

    let sess=(await supabase.from('session').select('*').order('id')).data??[];
    if(!sess.length && proj[0]){
      const { data } = await supabase.from('session')
        .insert({name:'First chat',project_id:proj[0].id}).select().single();
      sess = data ? [data] : [];
    }
    setSessions(sess);
  ...
  async function send(){
    ...
    const { data:user } = await supabase
      .from('message').insert({session_id:selected.id,role:'user',content:input}).select().single();
    if(!user) return;
    ...
    const { data:bot } = await supabase
      .from('message').insert({session_id:selected.id,role:'assistant',content:reply}).select().single();
    if(bot) setMessages(m=>[...m,bot]);
  }

  /* ---------- key handler  ★Enterで送信★ ---------- */
  ...
  const SessionItem=({s}:{s:Session})=>{
    ...
    return <li ref={n=>{if(n)drag(n)}} className="cursor-pointer py-1" onClick={()=>selectSession(s)}>{s.name}</li>;
  };
    ...
    return(
      <div ref={n=>{if(n)drop(n)}} className={`${card} mb-4 p-2`}>
        ...
        <div className="flex space-x-1 mt-2">
          <input value={tmpSess[p.id]||''}
            onChange={e=>setTmpSess({...tmpSess,[p.id]:e.target.value})}
            placeholder="新規セッション…" className="flex-1 border px-1 rounded"/>
          <button onClick={()=>addSession(p.id)} className={`${primary} text-white px-2 rounded`}>＋</button>
        </div>
      </div>
    );
  };
    ...
    <div className="flex h-screen">
      ...
      <aside className={`${primary} text-white w-80 p-4 space-y-3 relative`}>
        ...
        <div className="overflow-y-auto h-[calc(100vh-220px)] pr-1">
          {projects.map(p=><ProjectColumn key={p.id} p={p}/>)}
        </div>
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="flex p-3 border-t space-x-2">
          <input value={input} onChange={e=>setInput(e.target.value)}
            onKeyDown={handleKey}                        // ★ Enter handler
          ...
          <div className={`${card} w-80 p-6`}>
            ...
            <select value={cfg.provider}
              onChange={e=>{
                ...
                const def = p==='gemini'
                  ?'gemini-1.5-flash':p==='openai'
                  ?'gpt-3.5-turbo':p==='groq'
                  ?'llama3-70b-8192':'claude-3-haiku-20240307';
            ...
            <select value={cfg.model}
              ...
              {cfg.provider==='groq' && <option value="llama3-70b-8192">Llama‑3 70B</option>}
              {cfg.provider==='anthropic' && <option value="claude-3-haiku-20240307">Claude‑3 Haiku</option>}
            </select>

            {/* Key */}
            <label className="block text-sm mb-1">API Key</label>
            <input value={cfg.keys[cfg.provider]}
